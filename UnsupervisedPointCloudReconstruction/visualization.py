...
def visualize(args):
    ...
    if args.draw_original:
        save_path = os.path.join(save_root, args.dataset + '_' + args.split + str(args.item) + '_' + str(n) + '_origin.npy')
        color = [0.4, 0.4, 0.6]
        # mitsuba(pts.numpy(), save_path, color)
        np.save(save_path, pts.numpy())
    ...
    if one_model:
        ...
        for path in load_path:
            ...
            model.eval()
            reconstructed_pl, _ = model(pts.view(1, 2048, 3))
            save_path = os.path.join(save_root, model_name[:-4] + '_' + args.dataset + '_' + args.split + str(args.item) + '_' + str(n) + '.npy')
            # mitsuba(reconstructed_pl[0].detach().numpy(), save_path, clr=args.shape)
