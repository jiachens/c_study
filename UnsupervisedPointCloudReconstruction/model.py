...
class DGCNN_Cls_Encoder(nn.Module):
    ...
    def forward(self, x):        
        ...
        x = get_graph_feature1(x1, k=self.k)     # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
        x = self.conv2(x)                       # (batch_size, 64*2, num_points, k) -> (batch_size, 64, num_points, k)
        x2 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points)

        x = get_graph_feature1(x2, k=self.k)     # (batch_size, 64, num_points) -> (batch_size, 64*2, num_points, k)
        x = self.conv3(x)                       # (batch_size, 64*2, num_points, k) -> (batch_size, 128, num_points, k)
        x3 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 128, num_points, k) -> (batch_size, 128, num_points)

        x = get_graph_feature1(x3, k=self.k)     # (batch_size, 128, num_points) -> (batch_size, 128*2, num_points, k)
...
class DGCNN_Seg_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = get_graph_feature1(x, k=self.k)     # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
...
class FoldNet_Decoder(nn.Module):
    def __init__(self, args):
        ...
        if args.encoder == 'dgcnn':
            self.feat_dims = args.feat_dims * 2
        else:
...
class PCT_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = get_graph_feature1(x, k=32)
        x = self.seq1(x)
        x1 = self.pool1(x)

        x = get_graph_feature1(x1, k=32)
        x = self.seq2(x)
...
class PointNet_Encoder(nn.Module):
    def __init__(self, args):
        ...
        self.conv5 = nn.Conv1d(128, args.feat_dims, kernel_size=1, bias=False)
...
class DGCNN_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)
        x2 = F.adaptive_avg_pool1d(x, 1).view(batch_size, -1)
        x = torch.cat((x1, x2), 1).view(batch_size, 1, -1)
...
class ReconstructionNet(nn.Module):
    def __init__(self, args):
        ...
        if args.encoder == 'pointnet_simple':
            self.encoder = PointNet_Encoder(args)
        elif args.encoder == 'dgcnn':
            self.encoder = DGCNN_Encoder(args)
        elif args.encoder == 'pct':
            self.encoder = PCT_Encoder(args)
