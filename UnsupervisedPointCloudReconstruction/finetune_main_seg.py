...
LastEditTime: 2021-04-03 22:20:14
'''
...
from model_finetune import PointNet_Simple_Seg, DGCNN_Seg, Pct_Seg
import numpy as np
...
def _init_():
    if not os.path.exists(args.pre_path +'finetune_checkpoints_seg'):
        os.makedirs(args.pre_path +'finetune_checkpoints_seg')
    if not os.path.exists(args.pre_path +'finetune_checkpoints_seg/'+args.exp_name):
        os.makedirs(args.pre_path +'finetune_checkpoints_seg/'+args.exp_name)
    if not os.path.exists(args.pre_path +'finetune_checkpoints_seg/'+args.exp_name+'/'+'models'):
        os.makedirs(args.pre_path +'finetune_checkpoints_seg/'+args.exp_name+'/'+'models')
    os.system('cp finetune_main_seg.py '+args.pre_path+'finetune_checkpoints_seg'+'/'+args.exp_name+'/'+'finetune_main_seg.py.backup')
    os.system('cp model_finetune.py '+args.pre_path+'finetune_checkpoints_seg' + '/' + args.exp_name + '/' + 'model_finetune.py.backup')
    os.system('cp util.py '+args.pre_path+'finetune_checkpoints_seg' + '/' + args.exp_name + '/' + 'util.py.backup')
    os.system('cp data.py '+args.pre_path+'finetune_checkpoints_seg' + '/' + args.exp_name + '/' + 'data.py.backup')
    os.system('cp attack.py '+args.pre_path+'finetune_checkpoints_seg' + '/' + args.exp_name + '/' + 'attack.py.backup')
...
def train(args, io):
    ...
    if args.model == 'pointnet_simple':
        model = PointNet_Simple_Seg(args,output_channels=output_channel).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0 or epoch == args.epochs-1:
            if epoch == args.epochs-1:
                ...
                torch.save(best_model.state_dict(), args.pre_path+'finetune_checkpoints_seg/%s/models/model_best.t7' % (args.exp_name))
            ...
            torch.save(model.state_dict(), args.pre_path+'finetune_checkpoints_seg/%s/models/model_epoch%d.t7' % (args.exp_name,epoch))
    return model
...
if __name__ == "__main__":
    ...
    io = IOStream(args.pre_path+'finetune_checkpoints_seg/' + args.exp_name + '/run.log')
    io.cprint(str(args))
