...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            ...
            # print(seg_np.shape,pred_np.shape)
        ...
        if epoch % 10 == 0 or epoch == args.epochs-1:
            if epoch == args.epochs-1:
                ...
                torch.save(best_model.state_dict(), args.pre_path+'finetune_seg_checkpoints/%s/models/model_best.t7' % (args.exp_name))
            ...
            torch.save(model.state_dict(), args.pre_path+'finetune_seg_checkpoints/%s/models/model_epoch%d.t7' % (args.exp_name,epoch))
    return model
...
def test(args, io,model=None, dataloader=None):
    ...
    outstr = 'Test loss: %.6f, test acc: %.6f, test avg acc: %.6f, test iou: %.6f' % (
                                                                                            test_loss*1.0/count,
                                                                                            test_acc,
                                                                                            avg_per_class_acc,
                                                                                            np.mean(test_ious))
...
if __name__ == "__main__":
    ...
    io = IOStream(args.pre_path+'finetune_seg_checkpoints/' + args.exp_name + '/run.log')
    io.cprint(str(args))
